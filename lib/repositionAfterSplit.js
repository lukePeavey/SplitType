...
export default function repositionAfterSplit(element, settings) {
  ...
  // collect word and character elements into arrays
  // TODO find a better way to do this
  forEach(nodes, (node) => {
    words = types.words && Data(node).isWord ? [...words, node] : words
    chars = Data(node).isChar ? [...chars, node] : chars
  })

  if (!types.lines && !settings.absolute) {
    // -> If not splitting text into lines are using absolute position,
    //    return here.
