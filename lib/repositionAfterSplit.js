...
export default function repositionAfterSplit(element, settings, scrollPos) {
  ...
  if (settings.absolute) {
    ...
    data.set(element, 'cssWidth', element.style.width)
    data.set(element, 'cssHeight', element.style.height)
  }
  ...
  toArray(nodes).forEach((node) => {
    ...
    if (settings.absolute) {
      ...
      data.set(node, 'top', top)
      data.set(node, 'left', left)
      data.set(node, 'width', width)
      data.set(node, 'height', height)
    }
  }) // END LOOP
  ...
  if (types.lines) {
    ...
    lines = wordsInEachLine.map((wordsInThisLine) => {
      ...
      if (settings.absolute) {
        data.set(lineElement, 'height', lineDimensions.height)
        data.set(lineElement, 'top', lineDimensions.top)
      }
