...
export default function repositionAfterSplit(element, settings, scrollPos) {
  ...
  toArray(nodes).forEach((node) => {
    ...
    // Get the size and position of all split text nodes.
    if (settings.absolute) {
  ...
  if (types.lines) {
    ...
    lines = wordsInEachLine.map((wordsInThisLine) => {
      ...
      if (settings.absolute) {
        Data(lineElement).height = lineDimensions.height
        Data(lineElement).top = lineDimensions.top
      }
  ...
  if (settings.absolute) {
    ...
    toArray(nodes).forEach((node) => {
      const { isLine, top, left, width, height } = Data(node)
      const parentNode = Data(node.parentElement)
      const isChildOfLineNode = !isLine && parentNode.isLine
      ...
      node.style.top = `${isChildOfLineNode ? top - parentNode.top : top}px`
